...
insert into
    zona (nomZona, longitud, latitud)
values (
        "Palma de Mallorca",
        123132,
        1231451
    ), ("Es Pla", 45747, 2352352), ("Mitjorn", 34533, 54745), ("Es Raiguer", 345723, 343457), (
        "Serra de Tramuntana",
        23683,
        93453
    ), ("Llevant", 23576, 43363);
...
CREATE PROCEDURE AVISA(IN IDCONTROLER, IN IDITEM) BEGIN 
DECLARE 
	declare varcursor CURSOR FOR
	SELECT idItem, data
	FROM comanda
	    inner join item ON comanda.idCom = item.idCom
	WHERE
	    DATEDIFF(data, NOW()) = 5;
	...
	LOOP
	insert into
	    avis (idContr, idItem, descripcio) VALUE (
	,
	,
	        "Your item delivered to the main storage has already expired"
	    );
	END LOOP;
	CLOSE varcursor;
	END;
...
BEGIN 
	...
	-- si son iguals i no existeix ja l'avísper a aquest item, vol dir que afegim avis
...
CREATE PROCEDURE XXX() BEGIN DECLARE 
	DECLARE DECLARE done INT DEFAULT FALSE;
	DECLARE a CHAR(16);
	DECLARE b, c INT;
